...
`FORBIDDEN`（禁抛）或 `ALLOWED`（放行），并给出可复核的整数几何证据；另支持
**连续航迹裁决**（2～100 个有序航点，检查航段是否穿入禁抛区，返回 `CLEAR` 或
`BLOCKED` 及首次接触证据）。
...
施工船在执行连续航迹前，需要确认相邻航点之间不会穿入禁抛区：逐点裁决会漏掉
两个放行航点之间的越界航段。本接口接收与裁决一致的 `region` 与 **2～100 个
有序航点** `waypoints`，按输入顺序检查每条航段，返回 `CLEAR` 或 `BLOCKED`。
...
畅通响应（`200`）：`"verdict": "CLEAR"` 且 `"contact": null`。

裁决规则：

- 禁抛区为区域内部与边界的并集（边界含顶点，与逐点裁决一致）；任一航段
  与禁抛区存在公共点即为**受限航段**，全航迹存在受限航段时 `BLOCKED`；
- 阻断时报告**最早受限航段**（`segment_index` 从 0 起，航段 `i` 为
  `waypoints[i] → waypoints[i+1]`）及该段**首次接触**：接触参数
  `t = t_num/t_den`（约分有理数，`0 <= t <= 1`，接触点
  `P(t) = A + t·(B−A)`）与接触坐标 `x_num/x_den`、`y_num/y_den`
  （约分有理数，整数坐标分母为 1），全程整数与有理数比较，无浮点；
- **起点已禁抛**（起点在区域内或边界上）时接触参数为零，接触点即起点；
  起点严格在内部时不存在边界边，`edge_index` 为 `null`；
- **共线贴边**（航段与某条边共线重叠）时接触点为**重叠起点**；
- 同一接触点命中多条边（即顶点，含擦过顶点）时 `edge_index` 取
  **最小输入边序号**（与逐点裁决的边界归因一致）；
- **零长度航段**（相邻航点相同）按单点规则处理：该点禁抛则阻断且参数为零，
  放行则该段畅通；
- 结论、接触参数与接触坐标与区域顶点顺/逆时针方向及末尾闭合写法无关
  （`edge_index` 随输入编号变化，但归属同一条几何边）；
- 航点数量越界（少于 2 或多于 100）、坐标非整数/越界、区域非法时，按统一
  错误信封**整单返回 422**，绝不返回部分结果；本接口不接收 `points`、
  `exclusion_margin_cm` 与 `permitted_pockets`，未声明字段同样整单 422。
...
- **面积汇总**：矩形外环加两个口袋核对 `net_area2` 面积守恒、顺/逆时针与闭合
  ...
  大坐标二倍面积整数精确；
- **连续航迹**：两端放行但中途穿区时定位首次入口（最早受限航段序号、接触参数、
  接触坐标、边序号）、擦过顶点与沿边航行稳定阻断（共线贴边取重叠起点、同点
  命中多边取最小边序号）、完全绕行返回 `CLEAR`、起点已禁抛参数为零（严格内部
  起点边序号为 `null`）、零长度航段按单点规则、顺/逆时针结论与接触坐标一致、
  大坐标接触参数与坐标约分有理数精确、航点数量（2～100）/坐标/区域非法与
  未声明字段整单 422；
- **随机性质测试**：`test_random_polygons_match_exact_reference` 生成大量随机星形简单
  ...
  （同距取最小序号）与约分后的平方距离分数；
  `test_random_tracks_match_fraction_reference` 对随机航迹（含顶点航点与多段
  折线）对拍最早受限航段序号、首次接触参数、接触坐标与边序号。
